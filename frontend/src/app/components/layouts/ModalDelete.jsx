...
const ModalDelete = ({ onCancelDelete, onConfirmDelete, msg }) => {
  ...
  return (
    <>
      <div className='fixed flex z-[9999] top-20 left-0 w-full justify-center'>
        <div className='relative bg-white w-full max-w-md max-h-full rounded-lg shadow-lg border border-zinc-400'>
          ...
          <div className='text-center p-3'>
            ...
            <h3 className='text-lg text-zinc-700 font-normal mb-2'>Yakin ingin menghapus data ini?</h3>
            {msg && <p className='text-xs mb-5 font-medium text-textSecondary'>sub kategori dari kategori tersebut akan ikut terhapus</p>}
            ...
            <button onClick={onCancelDelete} data-modal-hide="popup-modal" type="button" className="text-textPrimary hover:text-white bg-white hover:bg-hoverBgButton focus:ring-2 focus:outline-none focus:ring-ringFocusBtn rounded-lg border border-borderButton text-sm font-medium px-5 py-2.5 focus:z-10 mb-5">Tidak, batal</button>
          </div>
        </div>
      </div>
