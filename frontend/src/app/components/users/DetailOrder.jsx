...
import { useGetOrderByUserIdQuery, useGetSnapOrderByTransactionIdQuery } from '../../features/order/orderApiSlice'
...
const DetailOrder = () => {
  ...
  const getSnap = async () => {
    setIsloadingGetSnap(true)
    setBtnPay(false)
    try {
      const response = await dispatch(getSnapBySnapToken(order?.data[0]?.snap_token))

      if (response.isSuccess) {
        setIsloadingGetSnap(false)
  ...
  return (
    <>
      {isLoading || isLoadingGetSnap ? <LoadingSpinner /> : null}
